import { formatNumberShort, formatMoney, getNsDataThroughFile, getActiveSourceFiles } from '/fm/helpers.js';
...
const argsSchema = [
    ...
    ['disable-faction', ['netburners']], // Factions to omit from all data, stats, and calcs, (e.g.) if you do not want to purchase augs from them, or do not want to see them because they are impractical to join at this time
...
async function manageFilteredSubset(ns, outputRows, subsetName, subset, printList = undefined /* undefined => automatically print if sort order changed */, purchaseable = false) {
    ...
    /// PRINTOUT RECCOMENDATION FOR NEXT AUGMENTATION
    try {
        let optAugs = "Faction: " + outputRows[5].split('\n')[2].split('Faction: ')[1].split('Stats:')[0];
        ns.tprint("\nReccomended Next Aug: " + optAugs);
    } catch (e) { }
